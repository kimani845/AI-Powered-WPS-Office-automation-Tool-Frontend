...
import { Settings, Star, Upload, Download, X, MessageSquare, Bot, Zap, Globe, AlertTriangle, CheckCircle, Info } from 'lucide-react';

// Import the useApi hook and apiService
import { useApi } from './hooks/useApi';
import { DocumentRequest, ProcessRequest, TranslationRequest } from './types/api'; // Ensure TranslationRequest is used
...
function App() {
  ...
  const [apiKey, setApiKey] = useState(localStorage.getItem('apiKey') || ''); // Load from localStorage
  const [showApiKey, setShowApiKey] = useState(false);

  const { error, generateDocument, processCommand, checkServerHealth, clearError } = useApi();

  useEffect(() => {
    checkServerHealth(
      (result) => {
        console.log('Backend server health:', result.message);
        setResponseContent(`✓ Server Health: ${result.message}\n${responseContent}`);
      },
      (err) => {
        console.error('Backend server error:', err);
        setResponseContent(`❌ Server Connection Error: ${err}\n${responseContent}`);
      }
    );
  }, []);
  ...
  const handleCommandExecution = async () => {
    ...
    await processCommand(commandInput, uploadedFiles.map(f => f.name).join(', '),
      (result) => {
        const downloadUrlMatch = result.match(/(http:\/\/\S+\/download\/\S+\.docx)/);
        if (downloadUrlMatch) {
          setResponseContent(`✓ Command executed successfully: "${commandInput}"\n\n${result.replace(downloadUrlMatch[0], '')}`);
          setDownloadLink(downloadUrlMatch[0]);
        } else {
          setResponseContent(`✓ Command executed successfully: "${commandInput}"\n\n${result}`);
        }
        setUploadedFiles([]);
      ...
      (errorMessage) => {
        setResponseContent(`❌ Error processing command: ${errorMessage}`);
      }
    );
    setIsLoading(false);
  };
  ...
  const handleDocGenSubmit = async (e: React.FormEvent) => {
    ...
    await generateDocument(docForm,
      (result) => {
        const downloadUrlMatch = result.match(/(http:\/\/\S+\/download\/\S+\.docx)/);
        if (downloadUrlMatch) {
          setResponseContent(`✓ Document Generated Successfully!\n\n${result.replace(downloadUrlMatch[0], '')}`);
          setDownloadLink(downloadUrlMatch[0]);
        } else {
          setResponseContent(`✓ Document Generated Successfully!\n\n${result}`);
        }
      ...
      (errorMessage) => {
        setResponseContent(`❌ Error generating document: ${errorMessage}`);
      }
    );
    setIsLoading(false);
  };
...
📊 Ready for document summarization or analysis commands. (Content will be sent to backend with command)`);
    }
  }, []);
  ...
  const handleSaveSettings = () => {
    if (apiKey.trim()) {
      ...
      setResponseContent('✓ API Key saved successfully!');
    } else {
      localStorage.removeItem('apiKey'); // Remove if empty
      setResponseContent('⚠ API Key cleared.');
    }
    setShowSettingsModal(false);
  };

  // Handle Upgrade Action (Simulated)
  const handleUpgrade = () => {
    setResponseContent('✓ Upgrade initiated! Please visit https://x.ai/grok for details.');
    setShowUpgradeModal(false);
    // In a real scenario, this could trigger an API call to handle subscription
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
                ...
                <>
                  ...
                  {downloadLink && (
                    <div className="mt-4 p-4 bg-green-50 rounded-xl flex items-center gap-2">
                      ...
                      <span className="text-green-700">Download ready:</span>
                      <a
                        href={downloadLink}
                        download
                        className="ml-auto flex items-center gap-1 px-4 py-2 bg-green-500 text-white rounded-full hover:bg-green-600 transition-all"
                      >
            ...
            <form onSubmit={handleDocGenSubmit} className="space-y-6">
              ...
              <div className="flex flex-col lg:flex-row gap-3 pt-4 border-t border-gray-200">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 py-3 px-6 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-full font-medium hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 hover:scale-105 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Generate Document
                </button>
                <button
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-3xl">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-300"
                  placeholder="Enter your API key for LLM services"
                />
                <button
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-3xl">
            ...
            <div className="space-y-4 text-gray-700">
              ...
              <ul className="space-y-2 list-disc pl-5">
                ...
                <li>File processing up to 100MB</li>
              </ul>
              <button
                onClick={handleUpgrade}
                className="w-full py-3 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-xl hover:from-yellow-600 hover:to-orange-600 transition-all"
              >
                Upgrade Now - Visit x.ai/grok
              </button>
            </div>
          </div>
        </div>
